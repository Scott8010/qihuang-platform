    ...
    <Card className="border" style={{ borderColor: C.border }}>
      <CardHeader className="pb-1">
        <div className="flex items-center justify-between">
          <CardTitle className="text-[17px] flex items-center gap-2" style={{ color: C.primary }}>
      ...
      <CardContent className="pb-5">
        {n === 0 ? (
          <div className="h-[196px] flex flex-col items-center justify-center gap-2 text-[15px]" style={{ color: C.light }}>
            {loading ? (
              ...
              <>
                ...
                当前没有待办任务
                <span className="text-[13.5px]">待审知识、逾期账单与系统告警出现时会自动汇聚到这里</span>
              </>
            )}
          </div>
        ) : (
            ...
            {ordered.slice(0, 3).map((t, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="p-4">
                    <div className="flex items-center justify-between">
                      <span className="text-[13px] px-2 py-0.5 rounded-full font-medium" style={{ color: tone.color, background: tone.bg }}>
                        {t.type}
                      </span>
                      <span className="text-[13px]" style={{ color: isTop ? tone.color : C.light }}>{t.tag}</span>
                    </div>
                    <div className="mt-2.5 text-[16px] font-semibold truncate" style={{ color: isTop ? C.ink : C.light }}>
                      {t.title}
                    </div>
                    {isTop && (
                      <>
                        <div className="mt-1.5 text-[14px] leading-5 line-clamp-2" style={{ color: C.mid }}>
                          {t.desc}
                        </div>
                        <div className="mt-3 flex gap-2">
                          <Button size="sm" className="h-8 text-[14px]" style={{ background: C.primary }} onClick={() => go(t.page)}>
                            去处理 <ArrowRight className="w-3.5 h-3.5 ml-1" />
                          </Button>
                          {n > 1 && (
                            <Button size="sm" variant="outline" className="h-8 text-[14px]" style={{ borderColor: C.border, color: C.mid }} onClick={() => setStart((s + 1) % n)}>
        ...
        <div className="mt-2 flex items-center justify-between text-[13.5px]" style={{ color: C.light }}>
