...
export default function Tenants({ go }: { go: (p: string) => void }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        <div className="flex rounded-lg border bg-white p-1" style={{ borderColor: C.border }}>
          {tabs.map((t) => (
            <button
              key={t.id}
              onClick={() => setTab(t.id)}
              className="px-4 py-1.5 text-[15px] rounded-md transition-colors"
              style={{
        ...
        <div className="relative">
          ...
          <Input
            ...
            className="pl-9 w-60 text-[15px] bg-white"
        ...
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent className="sm:max-w-[680px] max-h-[88vh] overflow-y-auto">
            ...
            <div className="space-y-4 py-2 text-[15px]">
              {/* ── ① 机构信息 ── */}
              <div className="rounded-xl p-4 space-y-3" style={{ background: C.soft }}>
                <div className="text-[14px] font-semibold tracking-wider" style={{ color: C.primary }}>① 机构信息</div>
                ...
                <div className="space-y-1.5">
                  <Label>机构代号（code，可选）<span className="text-[13px]" style={{ color: C.light }}> · 留空自动生成 JGxxxx，替代复杂 uuid</span></Label>
                ...
                <div className="grid grid-cols-2 gap-3">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Select value={form.plan} onValueChange={(v) => setForm({ ...form, plan: v })}>
                      <SelectTrigger><SelectValue /></SelectTrigger>
                      <SelectContent>
                        {plans.length === 0 && <div className="px-3 py-2 text-[14px]" style={{ color: C.light }}>套餐加载中…</div>}
                        {plans.map((p) => (
                ...
                <div>
                  <Label>机构地址<span className="text-[13px]" style={{ color: C.light }}> · 前 4 项为行政区划，详细地址单独填</span></Label>
                  ...
                  />
                  {form.addressDetail && (
                    <div className="text-right text-[13px] mt-0.5" style={{ color: form.addressDetail.length >= 200 ? "#B03A2E" : C.light }}>
                ...
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    ...
                    maxLength={150}
                  />
                  <div className="text-right text-[13px]" style={{ color: form.orgIntro.length >= 150 ? "#B03A2E" : C.light }}>
              ...
              <div className="rounded-xl p-4 space-y-3" style={{ background: "#FBF4E4" }}>
                <div className="text-[14px] font-semibold tracking-wider" style={{ color: C.gold }}>
                  ② 资质证照
                  {form.scene === "MED" && <span className="ml-2 text-[13px] font-normal" style={{ color: "#B03A2E" }}>医疗场景：两证必传</span>}
                </div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                    <Label>营业执照{form.scene === "MED" ? <span style={{ color: "#B03A2E" }}> *</span> : <span className="text-[13px]" style={{ color: C.light }}>（选填）</span>}</Label>
                    <div className="flex items-center gap-2">
                      ...
                      />
                      <label htmlFor="lic-business" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[14px] font-medium cursor-pointer border"
                        style={{ background: "#fff", color: C.primary, borderColor: C.border }}>
                        <UploadCloud className="w-3.5 h-3.5" />
                        {uploading === "business" ? "上传中…" : "选择文件"}
                      </label>
                      {form.licenseBusinessName && (
                        <span className="inline-flex items-center gap-1 text-[14px] truncate max-w-[130px]" style={{ color: C.mid }} title={form.licenseBusinessName}>
                      ...
                      {!form.licenseBusinessName && <span className="text-[14px]" style={{ color: C.light }}>未上传</span>}
                    </div>
                  </div>
                  <div className="space-y-1.5">
                    <Label>医疗机构执业许可证{form.scene === "MED" ? <span style={{ color: "#B03A2E" }}> *</span> : <span className="text-[13px]" style={{ color: C.light }}>（医疗专用）</span>}</Label>
                    <div className="flex items-center gap-2">
                      <input
                      ...
                      />
                      <label htmlFor="lic-medical" className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[14px] font-medium cursor-pointer border"
                        style={{ background: "#fff", color: C.primary, borderColor: C.border }}>
                        <UploadCloud className="w-3.5 h-3.5" />
                        {uploading === "medical" ? "上传中…" : "选择文件"}
                      </label>
                      {form.licenseMedicalName && (
                        <span className="inline-flex items-center gap-1 text-[14px] truncate max-w-[130px]" style={{ color: C.mid }} title={form.licenseMedicalName}>
                          <FileText className="w-3.5 h-3.5 shrink-0" style={{ color: C.gold }} />
                      ...
                      {!form.licenseMedicalName && <span className="text-[14px]" style={{ color: C.light }}>未上传</span>}
                    </div>
                  </div>
                </div>
                <div className="text-[13px] leading-relaxed" style={{ color: C.light }}>
                  {form.scene === "MED"
              ...
              <div className="rounded-xl p-4 space-y-3" style={{ background: C.bg }}>
                <div className="text-[14px] font-semibold tracking-wider" style={{ color: C.primary }}>③ 套餐与增值服务</div>
                {(() => {
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="text-[13px]" style={{ color: planHas3d ? C.mid : "#A8A29E" }}>
                            {planHas3d
                      ...
                      {planHas3d ? (
                        <Badge variant="outline" className="border-amber-300 text-[13px] font-medium whitespace-nowrap" style={{ color: C.gold, background: "#fff" }}>
                          <CheckCircle2 className="w-3 h-3 mr-1" />已含 · 自动开通
                        </Badge>
                      ) : (
                        <Badge variant="outline" className="text-[13px] font-medium whitespace-nowrap" style={{ color: "#78716C", borderColor: "#E7E5E4", background: "#FAFAF9" }}>
              ...
              {/* ── ④ 联系人 ── */}
              <div className="rounded-xl p-4 space-y-3" style={{ background: C.soft }}>
                <div className="text-[14px] font-semibold tracking-wider" style={{ color: C.primary }}>④ 联系人信息</div>
                <div className="grid grid-cols-2 gap-3">
                  <div className="space-y-1.5">
                ...
                <div className="text-[13px]" style={{ color: C.light }}>座机示例：021-12345678（须带区号）· 国际：+86-021-12345678</div>
              </div>

              <div className="text-[14px] rounded-lg p-3" style={{ background: C.bg, color: C.mid }}>
      ...
      <Card className="border" style={{ borderColor: C.border }}>
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-[15px]" style={{ minWidth: 1180 }}>
              <thead>
                <tr className="border-b" style={{ borderColor: C.border, background: C.soft }}>
                  <th className="px-5 py-3 text-left text-[14px] font-semibold whitespace-nowrap" style={{ color: C.mid, letterSpacing: "0.04em", minWidth: 280 }}>租户</th>
                  <th className="px-3 py-3 text-left text-[14px] font-semibold whitespace-nowrap" style={{ color: C.mid, letterSpacing: "0.04em", minWidth: 96 }}>场景</th>
                  <th className="px-3 py-3 text-left text-[14px] font-semibold whitespace-nowrap" style={{ color: C.mid, letterSpacing: "0.04em", minWidth: 108 }}>套餐</th>
                  <th className="px-3 py-3 text-left text-[14px] font-semibold whitespace-nowrap" style={{ color: C.mid, letterSpacing: "0.04em", minWidth: 116 }}>机构 / 用户</th>
                  <th className="px-3 py-3 text-left text-[14px] font-semibold whitespace-nowrap" style={{ color: C.mid, letterSpacing: "0.04em", minWidth: 220 }}>月调用量 / 配额</th>
                  <th className="px-3 py-3 text-left text-[14px] font-semibold whitespace-nowrap" style={{ color: C.mid, letterSpacing: "0.04em", minWidth: 96 }}>3D 模块</th>
                  <th className="px-3 py-3 text-left text-[14px] font-semibold whitespace-nowrap" style={{ color: C.mid, letterSpacing: "0.04em", minWidth: 92 }}>状态</th>
                  <th className="px-3 py-3 text-left text-[14px] font-semibold whitespace-nowrap" style={{ color: C.mid, letterSpacing: "0.04em", minWidth: 116 }}>到期时间</th>
                  <th className="px-4 py-3 text-right text-[14px] font-semibold whitespace-nowrap" style={{ color: C.mid, letterSpacing: "0.04em", minWidth: 168 }}>操作</th>
                </tr>
              </thead>
              <tbody>
                {shown.map((t) => {
                  ...
                  return (
                    <tr key={t.id} className="border-b last:border-0 hover:bg-[#F8FAF9] transition-colors" style={{ borderColor: C.border }}>
                      ...
                      <td className="px-3 py-3.5 align-middle">
                        <div className="flex justify-between text-[13px] mb-1 whitespace-nowrap" style={{ color: over ? "#B03A2E" : C.light }}>
                      ...
                      <td className="px-3 py-3.5 align-middle whitespace-nowrap">
                        {t.module3d
                          ? <Badge variant="outline" className="border-amber-300 text-[13px] font-medium" style={{ color: C.gold }}>已开通</Badge>
                          : <span className="text-[14px]" style={{ color: C.light }}>—</span>}
                      </td>
                      <td className="px-3 py-3.5 align-middle whitespace-nowrap">
                        <span className={`inline-block px-2.5 py-1 rounded border text-[13px] font-medium ${statusMap[t.status].cls}`}>{statusMap[t.status].label}</span>
                      </td>
                      <td className="px-3 py-3.5 align-middle whitespace-nowrap text-[14px]" style={{ color: C.mid }}>{t.expires}</td>
                      <td className="px-4 py-3.5 align-middle text-right">
                        <div className="inline-flex items-center gap-1">
                          <Button variant="ghost" size="sm" className="h-7 px-2 text-[14px] hover:bg-[#EAF2EE]" style={{ color: C.primary }} onClick={() => setDetail(t)}>详情</Button>
                          <span className="w-px h-3" style={{ background: C.border }} />
                          <Button variant="ghost" size="sm" className="h-7 px-2 text-[14px] hover:bg-gray-100" style={{ color: C.mid }} onClick={() => go("billing")}>续费</Button>
                          <span className="w-px h-3" style={{ background: C.border }} />
                          <Button variant="ghost" size="sm" className="h-7 px-2 text-[14px] hover:bg-red-50" style={{ color: "#B03A2E" }} onClick={() => setDelTenant(t)}>删除</Button>
                        </div>
                      </td>
                    </tr>
                  );
      ...
      <div className="text-[14px]" style={{ color: C.light }}>共 {shown.length} 家租户 · 数据权限按 tenant_id 行级隔离</div>
