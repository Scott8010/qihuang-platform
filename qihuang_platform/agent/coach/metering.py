"""
coach · 计量埋点（对齐 health-advisor/metering 范式）
...
logger = logging.getLogger("coach.metering")
...
def check_quota(tenant_id: Optional[str]) -> bool:
    ...
    try:
        ...
        if not isinstance(res, dict) or res.get("success") is False:
            logger.warning("[coach.metering] check_quota 返回异常态，放行: %s", res)
            return True
        data = res.get("data") or {}
        if data.get("is_exceeded"):
            logger.info(
                "[coach.metering] 租户 %s 配额超额(%.1f%%)，应拦截",
                tenant_id, data.get("quota_percentage", 0),
            )
            return False
        return True
    except Exception as e:  # noqa: BLE001
        logger.warning("[coach.metering] check_quota 执行失败(DB未就绪?)，降级放行: %s", e)
        return True
...
async def record_call(
    *,
    ...
    latency_ms: float,
    trace_id: str,
    endpoint: str = "/api/v1/agent/coach/evaluate",
    status_code: int = 200,
    action: str = "evaluate",
) -> None:
    """记录一次 coach 调用（业务级计费埋点）。仅业务成功执行后调用。"""
    try:
