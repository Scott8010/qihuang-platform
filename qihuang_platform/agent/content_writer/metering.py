"""
content_writer · 计量埋点（对齐 coach/metering 范式）
...
def check_quota(tenant_id: Optional[str]) -> bool:
    ...
    try:
        ...
        if not isinstance(res, dict) or res.get("success") is False:
            logger.warning("[content_writer.metering] check_quota 返回异常态，放行: %s", res)
            return True
        data = res.get("data") or {}
        if data.get("is_exceeded"):
            logger.info(
                "[content_writer.metering] 租户 %s 配额超额(%.1f%%)，应拦截",
                tenant_id, data.get("quota_percentage", 0),
            )
            return False
        return True
    except Exception as e:  # noqa: BLE001
        logger.warning("[content_writer.metering] check_quota 执行失败(DB未就绪?)，降级放行: %s", e)
        return True
...
async def record_call(
    *,
    ...
    latency_ms: float,
    trace_id: str,
    endpoint: str = "/api/v1/agent/content-writer/generate",
    status_code: int = 200,
    action: str = "generate",
    variants: int = 1,
    token_used: int = 0,
